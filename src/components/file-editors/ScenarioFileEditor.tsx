...
import { MAX_PLAYER_CLANS } from '../../domain/clans';
import { createInitialGameState } from '../../domain/state';
import EventEditor from '../EventEditor';
import ScenarioMapStudio from './ScenarioMapStudio';
...
export default function ScenarioFileEditor({
  value,
  onChange,
  onImportAdventurers,
  onImportEvents,
  ...
  onSelectMapFile,
  showStatus
}: ScenarioFileEditorProps) {
  const adventurerInputRef = useRef<HTMLInputElement>(null);
  const eventInputRef = useRef<HTMLInputElement>(null);
  const [eventEditorBase] = useState(() => createInitialGameState({ isDmMode: true }));
  const [selectedMissionId, setSelectedMissionId] = useState<string | null>(value.events[0]?.id ?? null);
  const [workspaceTab, setWorkspaceTab] = useState<'MAP' | 'GRAPH' | 'EVENTS' | 'WORLD'>('MAP');
  const patch = (change: Partial<ScenarioFileData>) => onChange({ ...value, ...change });

  useEffect(() => {
    if (selectedMissionId === null) return;
    if (selectedMissionId && value.events.some(event => event.id === selectedMissionId)) return;
    setSelectedMissionId(value.events[0]?.id ?? null);
  }, [selectedMissionId, value.events]);

  const eventEditorState: GameState = {
    ...eventEditorBase,
    day: 1,
    nClans: value.nClans,
    guildName: value.guildName,
    guildShortName: value.guildShortName,
    hCost: value.hCost,
    themeId: value.themeId,
    mapWidth: value.mapWidth,
    mapHeight: value.mapHeight,
    spawnPolygon: value.spawnPolygon,
    mapRegions: value.mapRegions,
    mapEffectsEnabled: value.mapEffectsEnabled,
    hqPos: value.hqPos,
    clans: value.clans,
    adventurers: value.adventurers,
    missions: value.events,
    allMissions: value.events,
    scenarioChains: value.chains ?? [],
    contracts: [],
    history: [],
    completedMissionIds: []
  };

  const updateEventEditorState = (change: Partial<GameState>) => {
    const events = change.allMissions ?? change.missions;
    if (events || change.scenarioChains) patch({ ...(events ? { events } : {}), ...(change.scenarioChains ? { chains: change.scenarioChains } : {}) });
  };

  const setInitialActiveClanCount = (nClans: number) => {
    let remaining = nClans;
    const clans = value.clans.map(clan => {
      if (clan.id === 'clan_guild') return clan;
      const isActive = remaining > 0;
      remaining -= 1;
      return { ...clan, isActive };
    });
    patch({ nClans, clans });
  };
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-5 p-4 sm:p-6">
      <nav className="sticky top-[78px] z-[70] flex flex-wrap gap-2 rounded-xl border border-emerald-500/20 bg-black/95 p-2 shadow-xl backdrop-blur" aria-label="Разделы Студии кампании">
        <WorkspaceTab active={workspaceTab === 'MAP'} onClick={() => setWorkspaceTab('MAP')} label="Карта" description="События, регионы и предпросмотр" />
        <WorkspaceTab active={workspaceTab === 'GRAPH'} onClick={() => setWorkspaceTab('GRAPH')} label="Граф" description="Цепочки, дни и зависимости" />
        <WorkspaceTab active={workspaceTab === 'EVENTS'} onClick={() => setWorkspaceTab('EVENTS')} label={`События · ${value.events.length}`} description="Этапы, цепочки и награды" />
        <WorkspaceTab active={workspaceTab === 'WORLD'} onClick={() => setWorkspaceTab('WORLD')} label="Мир и данные" description="Экономика, кланы и наборы" />
      </nav>

      <div className={workspaceTab === 'MAP' ? 'block' : 'hidden'}>
        <ScenarioMapStudio
          value={value}
          mapUrl={mapUrl}
          ...
          onChange={onChange}
          onSelectMapFile={onSelectMapFile}
        />
      </div>

      <div className={workspaceTab === 'GRAPH' ? 'space-y-4' : 'hidden'}>
        <ScenarioGraphEditor events={value.events} chains={value.chains ?? []} selectedMissionId={selectedMissionId} onSelectedMissionIdChange={setSelectedMissionId} onOpenMission={id => { setSelectedMissionId(id); setWorkspaceTab('EVENTS'); }} onChange={events => patch({ events })} />
        <ScenarioAuditPanel value={value} onSelectMission={id => { setSelectedMissionId(id); setWorkspaceTab('EVENTS'); }} />
      </div>

      <div className={workspaceTab === 'WORLD' ? 'space-y-5' : 'hidden'}>
      ...
      <EditorSection title="Экономика и карта" icon={<Boxes className="h-4 w-4" />}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <NumberField label="Активных кланов" value={value.nClans} min={0} max={Math.min(MAX_PLAYER_CLANS, value.clans.filter(clan => clan.id !== 'clan_guild').length)} onChange={setInitialActiveClanCount} />
          ...
          {value.clans.map(clan => (
            <div key={clan.id} className="rounded-xl border border-neutral-800 bg-black/30 p-4">
              <div className="grid gap-3 md:grid-cols-[minmax(180px,1.4fr)_90px_100px_120px_repeat(4,100px)_auto] md:items-end">
                ...
                {clan.id !== 'clan_guild' && <ToggleField label="Активен на старте" checked={clan.isActive !== false} onChange={isActive => {
                  const clans = value.clans.map(item => item.id === clan.id ? { ...item, isActive } : item);
                  patch({ clans, nClans: clans.filter(item => item.id !== 'clan_guild' && item.isActive !== false).length });
                }} />}
                <button type="button" disabled={clan.id === 'clan_guild'} onClick={() => patch({ clans: value.clans.filter(item => item.id !== clan.id), nClans: Math.max(0, Math.min(value.nClans, value.clans.filter(item => item.id !== clan.id && item.id !== 'clan_guild').length)) })} className="mb-0.5 rounded p-2 text-rose-500 transition hover:bg-rose-500/10 disabled:opacity-20" title="Удалить клан"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          ))}
        </div>
        <button type="button" disabled={value.clans.filter(clan => clan.id !== 'clan_guild').length >= MAX_PLAYER_CLANS} onClick={() => { const clan = newClan(value.clans); patch({ clans: [...value.clans, clan], nClans: Math.min(MAX_PLAYER_CLANS, value.nClans + 1) }); }} className="mt-3 flex items-center gap-2 rounded border border-emerald-500/30 px-3 py-2 font-mono text-xs text-emerald-400 hover:bg-emerald-500/10 disabled:cursor-not-allowed disabled:opacity-30"><Plus className="h-4 w-4" /> Добавить клан</button>
      </EditorSection>
      ...
      <div className={workspaceTab === 'EVENTS' ? 'block' : 'hidden'}>
        <EventEditor
          state={eventEditorState}
          updateState={updateEventEditorState}
...
function WorkspaceTab({ active, onClick, label, description }: { active: boolean; onClick: () => void; label: string; description: string }) {
  return <button type="button" onClick={onClick} className={`min-w-[190px] flex-1 rounded-lg border px-4 py-2.5 text-left transition ${active ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-neutral-800 bg-black/30 hover:border-neutral-700'}`}><strong className={`block font-mono text-[10px] uppercase tracking-wider ${active ? 'text-emerald-300' : 'text-neutral-400'}`}>{label}</strong><span className="mt-1 block text-[10px] text-neutral-600">{description}</span></button>;
}
